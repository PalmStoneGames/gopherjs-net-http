...
func (c *Client) doFollowingRedirects(ireq *Request, shouldRedirect func(int) bool) (resp *Response, err error) {
	...
	if c.Timeout > 0 {
		...
		if !ok {
			return nil, fmt.Errorf("code.palmstonegames.com/gopherjs-net-http: Client Transport of type %T doesn't support CancelRequest; Timeout not supported", c.transport())
		}
	...
	for redirect := 0; ; redirect++ {
		...
		if resp, err = c.send(req); err != nil {
			if wasCanceled() {
				err = errors.New(err.Error() + " (Client.Timeout exceeded while awaiting headers)")
			}
			break
		}
...
func (b *cancelTimerBody) Read(p []byte) (n int, err error) {
	...
	if err == io.EOF {
		...
		return n, errors.New(err.Error() + " (Client.Timeout exceeded while reading body)")
	}
	return
}
