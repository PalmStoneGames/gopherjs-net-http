...
func (c *Cookie) String() string {
	...
	if len(c.Domain) > 0 {
		if validCookieDomain(c.Domain) {
			...
			fmt.Printf("code.palmstonegames.com/gopherjs-net-http: invalid Cookie.Domain %q; dropping domain attribute",
				c.Domain)
		}
	}
	if c.Expires.Unix() > 0 {
...
func sanitizeOrWarn(fieldName string, valid func(byte) bool, v string) string {
	...
	for i := 0; i < len(v); i++ {
		...
		fmt.Printf("code.palmstonegames.com/gopherjs-net-http: invalid byte %q in %s; dropping invalid bytes", v[i], fieldName)
		ok = false
		break
	}
	if ok {
